...
function findmostExp(cart) {
  for (let i = 0; i < cart.length; i++){

    if (cart[i].price > mostExp){
      mostExp = cart[i].price;
    }
  }
  console.log(mostExp);
}
...
function applyDiscount(total, percent) {
  let discounted = total - (total * percent / 100)
  console.log(`After ${percent}% discount: ${discounted}`)
}
